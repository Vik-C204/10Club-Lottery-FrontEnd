...
export default function FundLinkButton() {
    ...
    async function updateUIValues() {
        ...
        const link = linkFunction
            ? linkFunction._hex.toString()
            : "Wallet not connected to Ethereum"

        setLinkRate(
            link !== "Wallet not connected to Ethereum"
                ? ethers.utils.formatUnits(link, 8)
                : "Wallet not connected to Ethereum"
        )
        setSubBalance(
            balance !== "Wallet not connected to Ethereum"
    ...
    const { runContractFunction: getSub } = useWeb3Contract({
        abi: abi,
        ...
        <ScaleFade whileHover={{ scale: 1.1 }} initialScale={0.001} in={enterCount > 0}>
            ...
            >
                ...
                <Center mb={"2vh"}>
                    {isSmallerThan721 ? (
                        <VStack>
                            <Heading mr={"2vh"} color={"black"} size={"lg"}>
                                {"Balance of LINK:" + " "}
                            </Heading>{" "}
                            <Heading
                                mr={"3vh"}
                            ...
                            <Heading ml={"3vh"} mr={"2vh"} color={"black"} size={"lg"}>
                                Current LINK/USD:
                            </Heading>
                            <Heading
                                bgGradient={"linear(to-r, red.500, yellow.500)"}
                ...
                {isSmallerThan721 ? (
                    <Center>
                        <Box>
                            <Text align={"center"} fontSize={"lg"}>
                                Make sure to approve both transactions, the initial one, sending
                                LINK to this lottery and the final one that will appear within a
                                minute of approving the first, sending LINK from this lottery to
                                ChainLINK VRF to get a truly random number.
                            </Text>
                            <Box
                                width={"100%"}
                                height={"2px"}
                    ...
                    <>
                        ...
                        <Center>
                            <Box>
                                <Text fontSize={"lg"}>
                                    of approving the first, sending LINK from this lottery to
                                    ChainLINK VRF to get a truly random number.
                                </Text>
                                <Box
                                    width={"100%"}
                                    ...
                                    <Field name="fee" validate={validateName}>
                                        {({ field, form }) => (
                                            ...
                                            >
                                                ...
                                                <Input
                                                    {...field}
                                                    placeholder="Amount of LINK you wish to fund the lottery with"
                                                />
                                            </FormControl>
                                        )}
                                    </Field>
